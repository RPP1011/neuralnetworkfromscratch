...
use crate::{layers::layers::layers::Layer, math::{tensor_context::{TensorContext, TensorRef}}};
...
impl Model for Sequential {
    ...
    fn compile(&mut self, input_shape: Vec<usize>, _output_shape: Vec<usize>, _optimizer: Optimizer, _loss: LossFunction, _metrics: Vec<Metric>) {
        // Iterate thropugh all layers and compile them
        let dummy = self.context.borrow_mut().new_tensor(input_shape.clone(), vec![0.0; input_shape.iter().product()]);
    ...
    fn fit(&mut self, data_tensor:TensorRef, _labels:TensorRef, epochs: usize) {
